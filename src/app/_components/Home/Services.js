...
export default function Services() {
  ...
  const servicesWithPreview = SERVICES_DATA.map((service) => ({
    ...service,
    previewText:
      service.content.length > MAX_PREVIEW_LENGTH
        ? `${service.content.slice(0, MAX_PREVIEW_LENGTH)}...`
        : service.content,
    needsTruncation: service.content.length > MAX_PREVIEW_LENGTH,
  }));
    ...
    <main className="py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {servicesWithPreview.map(
            ({ title, content, anchor, icon, previewText, needsTruncation }) => (
              <motion.article
                key={title}
                id={anchor}
                initial={{ opacity: 0, y: 20 }}
